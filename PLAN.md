...
**Авто-зум по кліках** (Screen Studio `mouseclicks-0.json`): `--clicks <path>` → punch-in
до точки кліку в сегментах де є клік (`--zoom-factor 1.28`). **Фонова музика**: `--music <mp3>
--music-vol 0.05` (тиха, циклічна, зводиться з голосом). Приклад повного прогону:
`node render.js --clicks .../mouseclicks-0.json --music library/sfx/the-feeling.mp3 --music-vol 0.05 --out ~/Movies/recut.mp4`
